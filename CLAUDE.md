...
A meal planning and fridge inventory manager structured as a Hermes plugin. The entry point is `__init__.py:register(ctx)`, which auto-discovers tool handlers under `src/handlers/`, registers the inventory-awareness hook, and installs the skill. All state is persisted in JSON files under `data/`.
...
- **`json_fridge.py`** — `JsonFridgeRepository` (`<data_dir>/fridge.json`, versioned `schema_version: 6` structured inventory with aliases, category metadata, soft availability, internal stock cycles, recipe-only catalog identities, a backward-compatible name projection, and atomic legacy/v2–v5 migration). Its re-entrant advisory file lock protects agent/Web read-modify-write sequences across processes; `remove_items(items)` is the delta-rollback for finalize.
- **`json_history.py`** — `JsonHistoryRepository` (`<data_dir>/history.json`, schema v2 stable cooking occurrences). Legacy native and Web histories migrate deterministically; corrections retain rows with `retracted_at`, and compatibility reads derive latest active cook dates.
- **`json_receipt.py`** — `JsonReceiptRepository` (`<data_dir>/receipts.json`, schema v1 append-preserving purchase receipts). Strict reads fail closed; public native mutations use the common descriptor-pinned audited transaction lock, evidence/semantic deduplication, OCC corrections, and stable receipt/line IDs.
...
- `fridge.json` — Versioned structured kitchen inventory (`{"schema_version": 6, "items": [...]}`); each item carries `category`, `available`, `ever_stocked`, normalized `aliases`, and an internal monotonic `stock_cycle`. Legacy/v2/v3/v4/v5 inputs read compatibly and migrate on first mutation. Canonical names and aliases share one identity namespace, so generic recipe ingredients resolve to and consume the exact product identity. Schema v6 fails closed unless `available => ever_stocked`; `ever_stocked` is monotonic, and `stock_cycle` changes only on available→unavailable. Both fields are hidden from public mutation schemas. Web category/replenish/edit/delete uses `expected_updated_at` OCC under the inventory lock, while native tools are serialized latest-intent operations.
- `history.json` — Versioned canonical cooking occurrences with stable IDs, plan links, actual portions/yield, precision-aware cook time, and correction metadata.
- `receipts.json` — Versioned purchase-receipt ledger with immutable revisions, ordered raw lines, exact integer cents, evidence hashes, explicit review/retraction states, and analytical links. Receipt mutations never imply inventory or shopping mutations.
- `shopping_requests.json` — Active manual/derived receipt records, durable exact-name reservations, and completed tombstones. Reconciliation is serialized by its cross-process lock: reserve winner → inventory write → completion.
...
- **Cooking lifecycle**: `register_cooked_meal` consumes essential inventory/prep while preserving the linked plan row and transitioning it from `planned` to `cooked`. Plan/history/inventory/prep are one audited recovery-safe command. Metadata correction for the same physical cook passes `replaces_event_id` and the occurrence OCC revision to the same command: the predecessor is atomically retracted, one replacement remains active, omitted metadata is inherited, and inventory/prep/tuning side effects are not repeated. Plain re-registration after prior cooking history fails closed.
- **Audit ordering**: New canonical `prepare.json` records use schema v2 and carry `predecessor_transaction_id`, so committed transaction order is append-linked rather than inferred from wall-clock timestamps. Schema-v1 records remain a legacy recovery format. Canonical transaction directories are authoritative; monthly JSONL is a validated, rebuildable projection.
