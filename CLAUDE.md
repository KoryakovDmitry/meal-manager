...
- `fridge.json` — Versioned structured kitchen inventory (`{"schema_version": 5, "items": [...]}`); each item carries `category`, `available`, `ever_stocked`, and normalized `aliases`. Legacy/v2/v3/v4 inputs read compatibly and migrate on first mutation. Canonical names and aliases share one identity namespace, so generic recipe ingredients resolve to and consume the exact product identity. Schema v5 fails closed unless `available => ever_stocked`; `ever_stocked` is monotonic and not publicly editable. Web category/replenish/edit/delete uses `expected_updated_at` OCC under the inventory lock, while native tools are serialized latest-intent operations.
