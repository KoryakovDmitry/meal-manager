...
Confirmed duplicate identities are repaired through the native-only `merge_product_identity` maintenance command, never through ordinary stock removal. The operation serializes against the receipt AUDIT manager before taking shopping-request and inventory locks, checks both source and target `updated_at` values under lock, requires an unavailable source and available same-category target, transfers the complete source name/alias namespace, preserves the target batch and `stock_cycle`, and removes the source in one atomic inventory write. A completed shopping receipt tombstone, active derived receipt, pending exact reservation, or analytical link in any append-only purchase-receipt revision that refers to the source makes the merge fail closed rather than creating a dangling identity or cross-file partial transaction.
...
- Detailed SHOP-1 contract: [`docs/issues/SHOP-1-live-shopping-and-receipt-reconciliation.md`](docs/issues/SHOP-1-live-shopping-and-receipt-reconciliation.md)
