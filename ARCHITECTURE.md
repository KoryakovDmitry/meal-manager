...
Confirmed duplicate identities are repaired through the native-only `merge_product_identity` maintenance command, never through ordinary stock removal. The operation locks shopping requests before inventory, checks both source and target `updated_at` values under lock, requires an unavailable source and available same-category target, transfers the complete source name/alias namespace, preserves the target batch and `stock_cycle`, and removes the source in one atomic inventory write. A completed receipt tombstone, active derived receipt, or pending exact reservation that refers to the source makes the merge fail closed rather than creating a cross-file partial transaction.
