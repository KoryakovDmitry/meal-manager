...
- Metadata correction for the same physical cook uses `register_cooked_meal` with explicit `occurrence_id`, current `expected_revision`, and `replaces_event_id`. It atomically retracts the predecessor and appends one active replacement without repeating inventory, prep, or tuning side effects. Omitted metadata is inherited; explicit null clears portion fields. Plain re-registration of an occurrence with prior cooking history fails closed.
- New audit `prepare.json` records use schema v2 and link the previous committed transaction through `predecessor_transaction_id`; this total order is independent of wall-clock timestamps. Legacy schema-v1 records remain readable only for backward-compatible recovery. Canonical transaction directories are append-only, and monthly JSONL is a strictly reconciled derived projection rather than authoritative state.
