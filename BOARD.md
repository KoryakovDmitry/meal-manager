...
**Product direction.** Meal row становится stable occurrence со status lifecycle (`planned`, `cooked`, `moved`, `substituted`, `skipped`, `cancelled`). Cooking переводит occurrence в `cooked`, но не удаляет её из сетки; Web показывает completed badge/timestamp и отличает настоящий empty day от закрытой occurrence. Move/substitution оставляют terminal source и создают linked successor через immutable root occurrence chain. Repeated dish cooking создаёт отдельные occurrences, planned/actual portions и yield/leftovers связываются явно.

**Audit foundation.** Design accepted after three independent read-only audits: canonical proof is a file-backed prepared transaction directory with before/after fingerprints and immutable `commit|abort|conflict` marker; monthly JSONL is only a rebuildable redacted analytics projection. Все successful native/Web mutations получают typed committed events с operation/entity identity, UTC time, actor/surface, correlation/causation, domain payload и correction/provenance metadata. Current mechanical inventory contains **38 native mutating handlers** and **19 mutating Web routes**; coverage gate must auto-discover both sets and reject every unregistered/bypassing writer.
...
**Fail-closed границы.** Missing/same identity, available source, unavailable target, category mismatch, stale source/target OCC, corrupt inventory/shopping/receipt schema и write failure не меняют inventory bytes. Merge блокируется при completion tombstone с source `product_id`, active derived receipt source occurrence, pending exact receipt в source namespace или analytical product/inventory link в любой append-only purchase-receipt revision; исторические references не retargetятся, потому что это потребовало бы отдельной audited migration. Recipe/prep labels не переписываются: полный продуктовый namespace переносится в aliases target.
...
**Результат.** Первый read-only срез выпущен в `c229962` (2026-09-10). `GET /api/receipts` (status/merchant/period фильтры, limit 1–1000, include_retracted, newest-first), `GET /api/receipts/{id}` (+include_revisions), `GET /api/purchase-analytics`; страница «Чеки» с фильтрами, analytics-строкой, списком со статус-badge и карточкой чека (ordered raw lines, reconciliation). Чтение — та же схема локов, что native (audit lock → recover → read_target → strict load); corrupt/symlink/FIFO ledger → sanitized 503; только GET (write → 405). Весь OCR/пользовательский текст XSS-safe. Автозагрузка при открытии вкладки и в refreshAll. Gate: RED→GREEN `web/test_web_receipts.py` 28/28; 299 unit, 654 integration, web plans, Chromium a11y, compileall, diff-check, static scan PASS; independent exact-tree review `GO` (+15 live API probes, включая malformed-ledger fail-closed на всех эндпоинтах). Live QA: `meal-web` перезапущен, legacy чек 13.08 виден через API (needs_review, €58,33). Осталось на следующие срезы: price-history view, include_retracted в UI, Web create/correct/retract/upload.
